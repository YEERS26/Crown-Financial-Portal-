...
// Cloudflare Pages Function — Neon HTTP API with separate credentials
...
function checkAuth(req, env) {
  return req.headers.get('Authorization') === `Bearer ${env.APP_SECRET}`;
}
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (defMatch && request.method === 'PUT') {
      const b = await request.json();
      const r = await sql(env,
        `UPDATE payment_definitions SET name=$1,category=$2,freq=$3,weekday=$4,day_of_month=$5,
         next_date=$6,amount=$7,credit_bal=$8,credit_max=$9,priority=$10,updated_at=NOW()
         WHERE id=$11 RETURNING *`,
        [b.name,b.category,b.freq,b.weekday||null,b.day_of_month||null,b.next_date||null,
         b.amount,b.credit_bal??null,b.credit_max??null,b.priority||'',defMatch[1]]);
      return json(r.rows[0]);
    }
    ...
    if (path === 'instances' && request.method === 'POST') {
      ...
      if (Array.isArray(b)) {
        ...
        const r = await sql(env,
          `INSERT INTO payment_instances (def_id,date,amount,status) VALUES ${vals}
           ON CONFLICT (def_id,date) DO NOTHING RETURNING *`,
          b.flatMap(r => [r.def_id,r.date,r.amount,r.status||'']));
        return json(r.rows);
      }
      const r = await sql(env,
        `INSERT INTO payment_instances (def_id,date,amount,status) VALUES ($1,$2,$3,$4)
         ON CONFLICT (def_id,date) DO UPDATE SET amount=EXCLUDED.amount,status=EXCLUDED.status RETURNING *`,
    ...
    if (path === 'instances/cascade' && request.method === 'PUT') {
      ...
      await sql(env,
        `UPDATE payment_instances SET amount=$1,updated_at=NOW() WHERE def_id=$2 AND status!='Paid' AND date>=$3`,
